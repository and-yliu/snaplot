...
export async function judge(input) {
    ...
    return chatWithSchema(JUDGE_MODEL, [
 ...
 */
export async function bardAnnounce(judgesExplanation, imagePath) {
    const base64Image = await encodeImageToBase64(imagePath);
    return chatWithSchema(BARD_MODEL, [
...
export async function judgeRound(input) {
    ...
    const winnerSubmission = input.submissions.find((s) => s.playerId === judgeResult.chosenPlayerId);
    if (!winnerSubmission) {
    ...
    const bardResult = await bardAnnounce(judgeResult.judgesExplanation, winnerSubmission.photoLocation);
    return {
